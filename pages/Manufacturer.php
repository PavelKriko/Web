...
$models = $pdo->query('SELECT Car.id,Name,Photo,BodyType,Transmision,Engine,Speed,Cost FROM Manufacturer INNER JOIN Car ON Car.ManufacturerID = Manufacturer.ID 
WHERE ManufacturerName = "' . $manufacturer . '"')->fetchAll(PDO::FETCH_ASSOC);
                    ...
                    <div class="manufacter__models">
                        ...
                        <ul class="list__models">
                            <?php foreach($models as $model){
                                echo '<li>
                                <div class="model__name"><a class = "model__link" href ="/'.$query[0]['ManufacturerName'].'/'.$model['Name'].'">'.$model['Name'].'</a></div>
